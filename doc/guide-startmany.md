...
2. [Sending 5,000 Vulcano to new wallet addresses.](#option2)
...
### Send 5,000 Vulcano to New Addresses

Just like setting up a standard MN. Send exactly 5,000 Vulcano to each new address created above.
...
Copy the masternode private key and correspondig collateral output transaction that holds the 1K Vulcano.

The masternode private key may be an existing key from [Option 1](#option1), or a newly generated key from [Option 2](#option2). 

*Please note, the masternode priviate key is not the same as a wallet private key. Never put your wallet private key in the masternode.conf file. That is equivalent to putting your 5,000 Vulcano on the remote server and defeats the purpose of a hot/cold setup.*
...
[From the vulcano github repo](https://github.com/vulcano-crypto/vulcano/blob/master/doc/masternode_conf.md)
...
## What about the vulcano.conf file?

If you are using a masternode.conf file you no longer need the vulcano.conf file. The exception is if you need custom settings (thanks oblox). 

## Update vulcano.conf on server

If you generated a new masternode private key, you will need to update the remote vulcano.conf files.

Shut down the daemon and then edit the file.

```sudo nano .vulcano/vulcano.conf```
