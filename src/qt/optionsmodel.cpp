...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nDisplayUnit"))
        settings.setValue("nDisplayUnit", BitcoinUnits::VULC);
    ...
    if (!settings.contains("nAnonymizeVulcanoAmount"))
        settings.setValue("nAnonymizeVulcanoAmount", 1000);

    nObfuscationRounds = settings.value("nObfuscationRounds").toLongLong();
    nAnonymizeVulcanoAmount = settings.value("nAnonymizeVulcanoAmount").toLongLong();
...
// Wallet
    ...
    if (settings.contains("nAnonymizeVulcanoAmount"))
        SoftSetArg("-anonymizevulcanoamount", settings.value("nAnonymizeVulcanoAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in vulcano.cpp during shotdown to also remove the window positions
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const
{
    if (role == Qt::EditRole) {
        ...
        case AnonymizeVulcanoAmount:
            return QVariant(nAnonymizeVulcanoAmount);
        case Listen:
...
bool OptionsModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == Qt::EditRole) {
        ...
        case ObfuscationRounds:
            ...
            break;
        case AnonymizeVulcanoAmount:
            nAnonymizeVulcanoAmount = value.toInt();
            settings.setValue("nAnonymizeVulcanoAmount", nAnonymizeVulcanoAmount);
            emit anonymizeVulcanoAmountChanged(nAnonymizeVulcanoAmount);
            break;
        case CoinControlFeatures:
