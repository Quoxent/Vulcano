...
class TxViewDelegate : public QAbstractItemDelegate
{
    Q_OBJECT
public:
    TxViewDelegate() : QAbstractItemDelegate(), unit(BitcoinUnits::VULC)
    {
    }
...
void OverviewPage::setWalletModel(WalletModel* model)
{
    ...
    // update the display unit, to not use the default ("VULC")
    updateDisplayUnit();
}
...
void OverviewPage::updateObfuscationProgress()
{
    ...
    QString strAnonymizeVulcanoAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizeVulcanoAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if (currentBalance == 0) {
        ...
        strAnonymizeVulcanoAmount = strAnonymizeVulcanoAmount.remove(strAnonymizeVulcanoAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeVulcanoAmount + " / " + tr("%n Rounds", "", nObfuscationRounds);
    ...
    if (nMaxToAnonymize > nAnonymizeVulcanoAmount * COIN) nMaxToAnonymize = nAnonymizeVulcanoAmount * COIN;

    if (nMaxToAnonymize == 0) return;

    if (nMaxToAnonymize >= nAnonymizeVulcanoAmount * COIN) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                              .arg(strAnonymizeVulcanoAmount));
        strAnonymizeVulcanoAmount = strAnonymizeVulcanoAmount.remove(strAnonymizeVulcanoAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeVulcanoAmount + " / " + tr("%n Rounds", "", nObfuscationRounds);
    } else {
